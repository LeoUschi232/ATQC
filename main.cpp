...
int main(int argc, char *argv[]) {
  ...
  // Define options
  option long_options[] = {{"input", required_argument, nullptr, 'i'},
  ...
  int opt;
  while ((opt = getopt_long(argc, argv, "i:s:o:n:h", long_options, nullptr)) !=
         -1) {
    switch (opt) {
    case 'i':
  ...
  case 1:
    ...
    break;
  default:
    std::cerr << "Error: Unsupported stabilizer algorithm ID: " << stabilizer_id
              << std::endl;
    return 1;
  }
  ...
  try {
    if (input_filename.empty()) {
      ...
      std::cout << "Measurement of " << input_filename << " in progress..."
                << std::endl;
      for (unsigned int shot = 0; shot < num_shots; ++shot) {
        std::string measurement = StabilizerCircuit::executeCircuit(
            input_filename, *stabilizerTableau);
      ...
      for (size_t i = 0; i < sorted_results.size(); ++i) {
        output << "\"" << sorted_results[i].first
               << "\": " << sorted_results[i].second;
      ...
      if (!output_filename.empty()) {
        ...
        if (!out_file) {
          std::cerr << "Error: Unable to write to file: " << output_filename
                    << std::endl;
          return 1;
        }
        out_file << output.str();
...
void print_help(const char *program_name) {
  std::cout << "Usage: " << program_name << " [OPTIONS]\n"
            << "OPTIONS:\n"
            << "  -i, --input <input_filename>       Input file containing the "
               "circuit in QASM3 format.\n"
            << "  -s, --stabilizer <stabilizer-id>   Stabilizer algorithm ID "
               "(default: 1).\n"
            << "  -o, --output <output_filename>     Output file for "
               "measurement results.\n"
            << "  -n, --num-shots <num-shots>        Number of shots to "
               "execute (default: 1).\n"
            << "  -h, --help                         Display this help message "
               "and exit.\n";
}

void print_progress(const unsigned int current, const unsigned int total) {
  const int progress = static_cast<int>(100.0 * current / total);
  constexpr int bar_width = 50;
  const int pos = bar_width * progress / 100;
