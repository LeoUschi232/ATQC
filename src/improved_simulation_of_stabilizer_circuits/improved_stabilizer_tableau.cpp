...
void ImprovedStabilizerTableau::initializeTableau(const uint p_n) {
...
void ImprovedStabilizerTableau::rowsum(const uint h, const uint i) {
  const int rh = get_r(h);
  const int ri = get_r(i);

  int sum_g = 2 * (rh + ri);
  for (uint j = 1; j <= n; ++j) {
    const int xij = get_x(i, j);
    const int zij = get_z(i, j);
    const int xhj = get_x(h, j);
    const int zhj = get_z(h, j);
...
void ImprovedStabilizerTableau::CNOT(const uint control, const uint target) {
  if (control == 0) {
  ...
  const auto a = control;
  const auto b = target;
  for (int i = 1; i <= 2 * n; ++i) {
    const auto xia_zib = get_x(i, a) & get_z(i, b);
    const auto xib_zia_1 = get_x(i, b) ^ get_z(i, a) ^ 1;
...
void ImprovedStabilizerTableau::Hadamard(const uint qubit) {
  if (qubit == 0) {
  ...
  for (uint i = 1; i <= 2 * n; ++i) {
    const auto xia = get_x(i, a);
    const auto zia = get_z(i, a);
...
void ImprovedStabilizerTableau::Phase(const uint qubit) {
  if (qubit == 0) {
...
uint8_t ImprovedStabilizerTableau::Measurement(const uint qubit) {
  if (qubit == 0) {
  ...
  if (p <= 2 * n) {
    ...
    // In this case the measurement outcome is random, so the state needs to be
    // updated. This is done as follows. First call rowsum(i,p) for all i ∈ {1
    // to 2*n} such that i=/=p and xia = 1. However, this description of the
    // procedure is incomplete. The invariants of the algorithm are
    ...
    // This means that sum_g within the rowsum subroutine may be congruent to 1
    // or 3 if i=p-n due to invariant nr 2. This case is not mentioned in the
    // paper but must be taken into consideration.
    for (uint i = 1; i <= 2 * n; ++i) {
      if (i != p && i != p - n && get_x(i, a) == 1) {
        rowsum(i, p);
      }
    }
  ...
  // In this case the outcome is determinate, so measuring the state will not
  // change it. The only task is to determine whether 0 or 1 is observed. This
  // is done as follows. First set the (2n+1)st row to be identically 0.
  using_scratch_space = true;
  for (int j = 1; j <= n; ++j) {
    set_x(2 * n + 1, j, 0);
    set_z(2 * n + 1, j, 0);
  ...
  const auto measurement = get_r(2 * n + 1);
  using_scratch_space = false;
  return measurement;
}

void ImprovedStabilizerTableau::set(const uint index, const uint8_t value) {
  const uint byte_index = index / 8;
  const uint bit_index = index % 8;
  tableau[byte_index] &= ~(1 << bit_index);
  tableau[byte_index] |= value << bit_index;
}

uint8_t ImprovedStabilizerTableau::get(const uint index) const {
  const uint byte_index = index / 8;
  const uint bit_index = index % 8;
  return tableau[byte_index] >> bit_index & 1;
}

void ImprovedStabilizerTableau::set_x(const uint i, const uint j,
                                      const uint8_t x) {
...
void ImprovedStabilizerTableau::set_z(const uint i, const uint j,
                                      const uint8_t z) {
  if (i == 0 || j == 0 || i > 2 * n || j > n) {
...
void ImprovedStabilizerTableau::set_r(const uint i, const uint8_t r) {
  if (i == 0 || i > 2 * n) {
...
uint8_t ImprovedStabilizerTableau::get_x(const uint i, const uint j) const {
  if (i == 0 || j == 0 || i > 2 * n || j > n) {
...
uint8_t ImprovedStabilizerTableau::get_z(const uint i, const uint j) const {
  if (i == 0 || j == 0 || i > 2 * n || j > n) {
...
uint8_t ImprovedStabilizerTableau::get_r(const uint i) const {
  if (i == 0 || i > 2 * n) {
...
void ImprovedStabilizerTableau::throw_invalid_argument(
