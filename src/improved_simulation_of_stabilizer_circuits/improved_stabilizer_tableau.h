...
/**
 * Improved stabilizer tableaus for a new simulation algorithm, by which both
 * deterministic and random measurements can be performed in O (n^2) time. The
 * cost is a factor-2 increase in the number of bits needed to specify a state.
 * For in addition to the n stabilizer generators, we now store n “destabilizer”
 * generators, which are Pauli operators that together with the stabilizer
 * generators generate the full Pauli group Pn. So the number of bits needed is
 * 2*n*(2*n+1)=4*n^2+2*n.
 */
class ImprovedStabilizerTableau : public StabilizerTableau {
  /**
   * It will be convenient to add an additional (2n + 1)st row for scratch
   * space. Using this scratch space allows one to override valid qubit index
   * checks. Use only when performing a measurement.
   */
  bool using_scratch_space = false;

  /**
   * The algorithm uses a subroutine called rowsum (h, i), which sets generator
   * h equal to i + h. Its purpose is to keep track, in particular, of the phase
   * bit rh, including all the factors of i that appear when multiplying Pauli
   * matrices.
   ...
   */
  uint8_t get(uint index) const;

  /**
   * Throws an invalid argument exception with the given message if the scratch
...
public:
   ...
   * The entries of the tableau will be default initialized to the state
   * |0〉^(⊗n). This state is represented by a tableau with its (2n)x(2n)
   * submatrix set to identity.
   ...
   * @param j Index of qubit.
   */
  uint8_t get_x(uint i, uint j) const;
   ...
   * @param j Index of qubit.
   */
  uint8_t get_z(uint i, uint j) const;

  /**
   * Get the value of the phase operator bit-pair for a qubit.
   * The r variable consists of 2 bits to encode the possible values: 1, i, -1,
   * -i.
   * @param i Index of the generator.
   */
  uint8_t get_r(uint i) const;
};
} // namespace CliffordTableaus
